  ...
  return (
    <>
      ...
      <section className="card mx" style={{ marginTop: 10, padding: 18, display: "flex", alignItems: "center", gap: 14 }}>
        <div className="avatar" style={{ width: 54, height: 54, borderRadius: 18, background: "var(--ice)", color: "var(--brand)", fontSize: 18 }}>{user.name.trim().charAt(0).toUpperCase()}</div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 17, fontWeight: 700, lineHeight: 1.3 }}>{user.name}</div>
          <div className="muted" style={{ fontSize: 13, marginTop: 3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user.email}</div>
        </div>
      </section>
      {last && (
        <section className="card mx" style={{ marginTop: 12, padding: "6px 18px" }}>
          <div className="row"><span>Утас</span><span>{fmtPhone(last.customer.phone)}</span></div>
      ...
      <p className="muted mx" style={{ fontSize: 13, marginTop: 22 }}>Асуулт байвал: <a href={`tel:${s.phone1}`} style={{ fontWeight: 700 }}>{fmtPhone(s.phone1)}</a></p>
    </>
  );
}
