...
export default async function CustomerPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const d = customerId ? await customerDetail(customerId) : null;
  if (!d) notFound();
  const c = d.customer;
    ...
    <>
      ...
      <section className="card mx" style={{ padding: 18, display: "flex", alignItems: "center", gap: 14 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 17, fontWeight: 700, lineHeight: 1.3 }}>{c.name}</div>
          <div className="muted" style={{ fontSize: 13, marginTop: 3, lineHeight: 1.45 }}>{fmtPhone(c.phone)} · {c.bag}-р баг, {c.street}, {c.unit}</div>
      ...
      <section className="mx minis" style={{ marginTop: 14 }}>
