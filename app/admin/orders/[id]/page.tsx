...
import { fmtMoney, fmtPhone, PAYMENT_LABEL, STATUS_LABEL, type Payment, type Status } from "@/lib/domain";
  ...
  return (
    <>
      <header className="pagehead">
        ...
        <div><h1>Захиалга #{o.id}</h1><div className="sub">{humanDateTime(o.createdAt)} · {o.source === "admin" ? "гараар бүртгэсэн" : "вэбээр"}</div></div>
      </header>
      ...
      <section className="card mx" style={{ padding: 18, display: "flex", alignItems: "center", gap: 14 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div className="muted" style={{ fontSize: 13, marginTop: 3 }}>{fmtPhone(o.customer.phone)}</div>
        </div>
      ...
      <section className="card mx" style={{ marginTop: 12, padding: "6px 18px" }}>
        <div className="row"><span>Төлөв</span><span><span className={`pill-s st-${status}`}>{STATUS_LABEL[status]}</span></span></div>
        ...
        <div className="row total"><span>Нийт</span><span>{fmtMoney(o.total)}</span></div>
