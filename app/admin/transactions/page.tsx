...
export default async function Transactions({ searchParams }: { searchParams: Promise<{ g?: string; ref?: string; f?: string; q?: string }> }) {
  ...
  return (
    <>
      <header className="pagehead" style={{ paddingBottom: 6 }}><div><h1>Гүйлгээ</h1><div className="muted" style={{ fontSize: 13, marginTop: 3 }}>Орлого ба төлбөрүүд · <Link href="/admin/reports">дэлгэрэнгүй тайлан</Link></div></div></header>
      ...
      <section className="mx grid2" style={{ marginTop: 12 }}><Kpis r={r} /></section>
      ...
      <div className="mx" style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        <div className="chips">{chip("all", "Бүгд")}{chip("cash", "Бэлэн")}{chip("transfer", "Данс")}{chip("debt", "Өр төлөлт")}</div>
        <form className="search" action="/admin/transactions">
          ...
          <input className="inp" name="q" type="search" defaultValue={sp.q ?? ""} placeholder="Нэр эсвэл утасны дугаар" aria-label="Хайх" />
        </form>
      </div>
      <section className="card mx" style={{ marginTop: 12, padding: "0 16px 6px" }}>
        {groups.length === 0 && <div className="muted" style={{ textAlign: "center", padding: "30px 0", fontSize: 14 }}>Гүйлгээ олдсонгүй</div>}
        {groups.map((g) => (
          <div key={g.day}>
            <div className="dayhead"><span>{humanDate(g.day)}</span><b>{fmtMoney(g.sum)}</b></div>
            {g.items.map((p) => (
              <Link key={p.id} href={`/admin/customers/${p.customerId}`} className="txrow" style={{ color: "inherit" }}>
                <div className={`txicon ${p.method}`}>{p.method === "cash" ? <IconCash size={19} /> : <IconBank size={19} />}</div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 700, fontSize: 14.5 }}>{p.customer.name}</div>
                  <div className="muted" style={{ fontSize: 12.5, marginTop: 2 }}>{p.orderId != null ? `Захиалга #${p.orderId}` : "Өр төлөлт"} · {ubTime(p.createdAt)}</div>
                </div>
