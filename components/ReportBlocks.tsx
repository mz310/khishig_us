...
export function PeriodBar({ base, gran, r, chartMode = false }: { base: string; gran: Gran; r: Report; chartMode?: boolean }) {
  ...
  return (
    <div className={chartMode ? "filterrow" : "mx stack"} style={chartMode ? undefined : { gap: 12, marginTop: 10 }}>
      <nav className="segbox white links" aria-label="Хугацааны нэгж">
        {GRANS.map((g) => <Link key={g} href={href(g, r.today)} className={`seg${g === gran ? " on" : ""}`} aria-current={g === gran ? "page" : undefined}>{GRAN_LABEL[g]}</Link>)}
      </nav>
      <div className="perbar">
        <Link href={href(gran, r.prevRef)} className="navbtn" aria-label="Өмнөх" aria-disabled={!r.canPrev} tabIndex={r.canPrev ? undefined : -1}><IconLeft /></Link>
        <div className="plabel" aria-live="polite">{r.period.label}{r.current ? " · одоо" : ""}</div>
        <Link href={href(gran, r.nextRef)} className="navbtn" aria-label="Дараах" aria-disabled={!r.canNext} tabIndex={r.canNext ? undefined : -1}><IconRight /></Link>
      </div>
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div className="legend" style={{ marginTop: 14 }}>
        <span><i className="k-transfer" />Данс <b>{fmtMoney(r.sums.transfer)}</b></span>
        <span><i className="k-cash" />Бэлэн <b>{fmtMoney(r.sums.cash)}</b></span>
      </div>
    </>
  );
}

export function Kpis({ r }: { r: Report }) {
  return (
    <>
      <div className="kpi"><div className="l">Захиалга</div><div className="v">{r.orders.orders.toLocaleString("en-US")}</div><div className="delta"><Delta now={r.orders.orders} prev={r.prev.orders.orders} /></div></div>
      <div className="kpi"><div className="l">Хүргэсэн баллон</div><div className="v">{r.orders.bottles.toLocaleString("en-US")}</div><div className="delta"><Delta now={r.orders.bottles} prev={r.prev.orders.bottles} /></div></div>
      <div className="kpi"><div className="l">Шинэ өр</div><div className="v" style={{ color: "var(--debt)" }}>{fmtMoney(r.orders.newDebt)}</div><div className="delta"><Delta now={r.orders.newDebt} prev={r.prev.orders.newDebt} upIsGood={false} /></div></div>
      <div className="kpi"><div className="l">Өр төлөлт</div><div className="v" style={{ color: "var(--ok-ink)" }}>{fmtMoney(r.sums.repaid)}</div><div className="delta"><Delta now={r.sums.repaid} prev={r.prev.sums.repaid} /></div></div>
    </>
  );
}
